...
const data = new SlashCommandBuilder()
  ...
  .setDescriptionLocalizations(localeCommand.setBonus.comDes)
  .addStringOption((option) =>
    option
      .setName("tasktype")
      .setDescription("task type name")
      .setDescriptionLocalizations(localeCommand.setBonus.taskTypeDes)
      .setRequired(true)
      .addChoices(...options)
  )
  .addIntegerOption((option) =>
    option
      ...
      .setDescriptionLocalizations(localeCommand.setBonus.taskIdDes)
      .setRequired(true)
      .setMinValue(0)
  )
  .addIntegerOption((option) =>
    option
      ...
      .setDescriptionLocalizations(localeCommand.setBonus.amountDes)
      .setRequired(true)
      .setMinValue(0)
  )
  .addStringOption((option) =>
    option
      ...
      .setDescriptionLocalizations(localeCommand.setBonus.startAtDes)
      .setRequired(true)
  )
  .addStringOption((option) =>
    option
      .setName("endat")
      .setDescription("YYYY-MM-DD JST")
      .setDescriptionLocalizations(localeCommand.setBonus.endAtDes)
      .setRequired(true)
  );
...
const execute: Command = async (interaction) => {
  ...
  try {
    ...
    if (!getBonusByTaskTypeAndTaskId.length) {
      await bonusDb.create({
        taskTypeId: getTaskTypeData[0].id,
        bonusTaskId: getTaskId,
        amount: getAmount,
      ...
      replyEmbed.setTitle(
        `${translate.createdBonus.replace("{getTaskType}", getTaskType).replace("{getTaskId}", getTaskId.toString())}`
      );
    ...
    // if in the table, return it
    replyEmbed.setTitle(
      `${translate.updatedBonus.replace("{getTaskType}", getTaskType).replace("{getTaskId}", getTaskId.toString())}`
    );
